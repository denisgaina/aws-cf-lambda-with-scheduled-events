...
aws s3 sync . s3://$S3_BUCKET_NAME/lambda

# validating template
CF_VALIDATION=$(aws cloudformation validate-template --template-url https://$S3_BUCKET_NAME.s3.us-east-1.amazonaws.com/lambda/lambda.yaml)
echo $CF_VALIDATION
